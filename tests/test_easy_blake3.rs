...
//!
//! Symmetric counterpart to `bindings/python/tests/test_blake3.py` —
//! same coverage shape (nonce-size sweep, single + triple roundtrip,
//! single + triple auth with tamper rejection, persistence sweep,
//! plaintext-size sweep) applied to the high-level [`itb::Encryptor`]
//! surface instead of the lower-level Seed / encrypt / decrypt path.
//!
...
//! the same axes test_easy_blake2{b,s} cover.
//!
//! Persistence rides on [`Encryptor::export`] /
//! [`Encryptor::import_state`] (JSON blob, single round-trip). The
...
const BLAKE3_HASHES: &[(&str, i32)] = &[("blake3", 256)];
