//! SipHash-2-4-focused Rust binding coverage.
//!
//! W128 counterpart to `test_aescmac.rs`. SipHash ships at a single
//! width (-128) and has no internal fixed key — its keying material is
//! the per-call seed components — so `expected_key_len("siphash24")`
//! is 0 and the persistence path stores empty bytes for the hash key.
//!
